 ...
 */
class PaypalPaymentsUniversal extends NonmerchantGateway {
	/**
	...
	private static $authors = array(
		array('name'=>"Mohamed Anouar Achoukhy - Naja7Host SARL ",'url'=>"http://www.naja7host.com"),
		array('name'=>".:: Support us  , donate by PayPal ::. ",'url'=>"https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=2FQKWFL73CV9N")		
	...
	public function __construct() {
		...
		Language::loadLang("paypal_payments_universal", null, dirname(__FILE__) . DS . "language" . DS);
	}
	...
	public function getName() {
		return Language::_("PaypalPaymentsUniversal.name", true);
	}
	...
	public function getSettings(array $meta=null) {
        ...
        $getAcceptedCurrencies =  $this->getAcceptedCurrencies() ;      
		$select_options = array_combine($getAcceptedCurrencies, $getAcceptedCurrencies);
		...
		$rules = array(
			'account_id' => array(
				'valid' => array(
					...
					'message' => Language::_("PaypalPaymentsUniversal.!error.account_id.valid", true)
				)
			),
			'default_currency' => array(
				'empty' => array(
					'rule' => "isEmpty",
					'negate' => true,
					'message' => Language::_("PaypalPaymentsUniversal.!error.default_currency.empty", true)
				)
			),			
			'dev_mode'=>array(
				'valid'=>array(
					...
					'message'=>Language::_("PaypalPaymentsUniversal.!error.dev_mode.valid", true)
				)
			)
		);
	...
	public function buildProcess(array $contact_info, $amount, array $invoice_amounts=null, array $options=null) {
		...
		$this->attention_repeat = true ;
		$this->attention_notice = false ;
		$this->currency_original  = $this->currency ; 
		Loader::loadModels($this, array("Currencies"));
		// gate rates 
		if (!in_array($this->currency, $this->getAcceptedCurrencies() , true )) {
			$amount = $this->Currencies->convert($amount, $this->currency, $this->ifSet($this->meta['default_currency']) , Configure::get("Blesta.company_id"));			
			
			$this->attention_notice = true ;
		}
		$this->currency_original  = $this->currency ; 
		// Convert currency 
		if (in_array($this->currency, $this->getAcceptedCurrencies() , true )) 
			...
			$this->currency = $this->ifSet($this->meta['default_currency']);
	...
	private function buildForm($post_to, $fields, $recurring=false  ) {
		...
		$message = null ;
		if ($this->attention_notice AND  $this->attention_repeat ) {
			$message = Language::_("PaypalPaymentsUniversal.!error.default_currency.changed", true , $this->currency_original , $this->currency) ;				
			$this->attention_repeat = false ;
		}	
		// $this->setMessage( "info", Language::_("PaypalPaymentsUniversal.!error.default_currency.changed", true)); 
		// Controller::flashMessage("notice", Language::_("PaypalPaymentsUniversal.!error.default_currency.changed", true), null, false);
